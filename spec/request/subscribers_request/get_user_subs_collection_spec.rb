...
RSpec.describe 'GetUserSubsCollection', type: :request do
  ...
  let(:subbed_user) { create(:user, id: 1) }

  let(:sub_user) { create(:user, first_name: 'Jarry') }
 
  let!(:relation) { create(:relation, relating_id: sub_user.id,  related_id: subbed_user.id, id: 1) }
